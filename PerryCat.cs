...
{
    public class PerryCat : Partiality.Modloader.PartialityMod
    {
        public static Slugcatypus perry;
        public PerryCat()
        {
            ModID = "PerryCat Slugcat";
            Version = "1.0";
            author = "laura#2871";
        }
    ...
    public class Slugcatypus : SlugBaseCharacter
    {
        #region Vars
        public Hook jollyLayerFixHook;

        public static AttachedField<Player, PerryVars> PVars = new AttachedField<Player, PerryVars>();
        #endregion Vars
        ...
        private void Alignments_hook(On.CreatureCommunities.orig_LoadDefaultCommunityAlignments orig, CreatureCommunities self, int saveStateNumber)
        {
        ...
        private void Bats_N_Melee_hook(On.Player.orig_Collide orig, Player self, PhysicalObject otherObject, int myChunk, int otherChunk)
        {
            ...
            if (!Get_PVars(self).justAPlatypus && (self.animation != Get_PVars(self).collisionState || !(Get_PVars(self).collisionObjects.Contains(otherObject.abstractPhysicalObject.ID))) && (otherObject is Creature creature) && !creature.dead)
            {
                ...
                {
                    case Player.AnimationIndex.GrapplingSwing: hitBonus = 1f; break;
                    case Player.AnimationIndex.RocketJump: hitBonus = 1f; break;
                    case Player.AnimationIndex.Roll: hitBonus = 0.25f; break;
        ...
        private void Water_Boost_hook(On.Player.orig_UpdateAnimation orig, Player self)
        {
            if (IsMe(self))
            {
                if (self.animation == Player.AnimationIndex.DeepSwim && !(self.grasps[0] != null && self.grasps[0].grabbed is JetFish && (self.grasps[0].grabbed as JetFish).Consious))
                {
        ...
        {
            if (base.IsMe(self.owner as Player))
            {
        ...
        {
            if (!IsMe(self.slOracleBehaviorHasMark.player))
            {
                orig(self);
                return;
            }    
    ...
    public class PerryStart : UpdatableAndDeletable
    {
